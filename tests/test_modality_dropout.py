...
# Channels 7,8: (x - 87) / 160
CH8_ZERO = (0.0 - 87.0) / 160.0  # sparse measurements
...
def count_modality_states(n_samples: int, modality_dropout_prob: float, sparse_dropout_given_dropout: float):
    ...
    for _ in range(n_samples):
        ...
        # Check sparse (channel 8) - if all values equal normalized zero
        ch8_all_zero = ((tensor[8] - CH8_ZERO).abs() < 1e-4).all()
        sparse_dropped = ch8_all_zero
